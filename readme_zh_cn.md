...
- **多后台推理支持**
    - 支持 ONNX / NCNN Vulkan / NVIDIA TensorRT 多种深度学习推理后端，兼容各类硬件。
...
- 参考这个 [`指南`](src/resources/for_release_only/python_portable/用conda创建py环境.md) 在项目根目录创建 `python/` 文件夹，用 `./python/python.exe` 运行脚本。
